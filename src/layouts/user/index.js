...
export default function Dashboard(props) {
  ...
  const getActiveRoute = (routes) => {
    ...
    for (let i = 0; i < routes.length; i++) {
      if (routes[i].collapse) {
        ...
        if (
          window.location.href.indexOf(routes[i].layout + routes[i].path) !== -1 && routes[i].layout === '/user'
        ) {
  ...
  const getRoutes = (routes) => {
    return routes.flatMap((route, key) => {
      ...
      return route.collapse ? getRoutes(route.items) : [];
    });
  };
  ...
  return (
    <Box>
      <Box>
        ...
        >
          <Sidebar routes={userRoutes}  {...rest} />
          <Box
            float="right"
            minHeight="100vh"
            height="100%"
            ...
            <Portal>
              <Box>
                <Navbar
                  ...
                  message={getActiveNavbarText(userRoutes)}
                  fixed={fixed}
                  {...rest}
                />
              </Box>
